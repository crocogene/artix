...
scan_target_repo() {
    ...
    repo_pkgs=$(pacman -Sl "$repo" 2>/dev/null | awk -v arch="$target_arch" '$4==arch {print $2}')
    ...
    for pkg in $repo_pkgs; do
        ...
        deps=$(sed 's/<none>//g; s/None//g' <<<"$deps")
        ...
        for dep in $deps; do
            ...
            # Info from target repo
            target_dep_info=$(pacman -Si "$repo/$dep_name" 2>/dev/null)
